...
def _build_json(ctx: ExportContext) -> dict[str, Any]:
    ...
    market_date = str(as_of)[:10] if as_of else ctx.report_date
    return {
        "schema_version": "a_share_v5.1",
...
def _summary_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# A股结构化行情底座（生成日 {payload['report_date']}，行情日 {payload.get('market_date', payload['as_of'])}）",
        "",
...
def _premarket_audio(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"这里是{payload['report_date']}生成的A股盘前跑步收听版，行情截至{payload.get('market_date', payload['as_of'])}收盘。",
        "第一部分，数据质量。",
    ...
    lines.extend([
        ...
        "最后看A股核心股是否出现公告增量。",
        "第四部分，今日重点波动观察。以下排序按昨日绝对涨跌幅排列，不代表推荐顺序。",
    ])
...
def _postmarket_audio(payload: dict[str, Any], source_markdown: str) -> str:
    valid = [
        ...
        if s["change_pct"] is not None and s["close"] is not None
    ]
    gainers = sorted(
        [s for s in valid if s["change_pct"] > 0],
    ...
    flat = [s for s in valid if s["change_pct"] == 0]
    core = _pool_stocks(payload, "核心池")
    watch = _pool_stocks(payload, "观察池")

    lines = [
        f"这里是{payload['report_date']}生成的A股盘后跑步收听版，"
        f"行情截至{payload.get('market_date', payload['as_of'])}收盘。",
        "第一部分，数据质量和市场结论。",
        f"本次结构化数据质量评级为{payload['data_quality']}，"
        f"有效行情{payload['market_summary']['available_quote_count']}只。",
        f"股票池中上涨{len([s for s in valid if s['change_pct'] > 0])}只，"
        f"下跌{len([s for s in valid if s['change_pct'] < 0])}只，"
        f"平盘{len(flat)}只。",
        "第二部分，今日上涨股票。",
    ]
    ...
    if gainers:
        ...
        if len(gainers) < 3:
            lines.append(f"今日股票池中只有{len(gainers)}只股票上涨，不强行补足三只。")
    else:
        lines.append("今日股票池中没有上涨股票。")

    lines.append("第三部分，今日下跌和风险股票。")
    if decliners:
        for stock in decliners:
            lines.append(
                f"{stock['name']}，收盘{_fmt_num(stock['close'])}元，"
                f"下跌百分之{_fmt_num(abs(stock['change_pct']))}，"
                f"技术状态为{stock['trend']}。"
            )
    else:
        lines.append("今日股票池中没有下跌股票。")

    lines.extend([
        "第四部分，盘前判断验证。",
        "请把盘前提出的海外映射、韩国半导体承接和A股核心股公告增量，"
        "与今日收盘后的主线强弱、量能和核心股表现逐项对照。",
        "第五部分，核心池。",
    ])

    for stock in core:
        direction = (
            "上涨" if (stock["change_pct"] or 0) > 0
            else "下跌" if (stock["change_pct"] or 0) < 0
            else "平盘"
        )
        pct = abs(stock["change_pct"] or 0)
        lines.append(
            f"{stock['name']}，收盘{_fmt_num(stock['close'])}元，"
            f"{direction}百分之{_fmt_num(pct)}，"
            f"五日均线{_fmt_num(stock['ma5'])}，"
            f"十日均线{_fmt_num(stock['ma10'])}，"
            f"二十日均线{_fmt_num(stock['ma20'])}，"
            f"当前判断为{stock['trend']}。"
        )

    lines.append("第六部分，观察池风险提示。")
    watch_sorted = sorted(
        watch,
        key=lambda s: abs(s["change_pct"] or 0),
        reverse=True,
    )
    for stock in watch_sorted[:5]:
        direction = (
            "上涨" if (stock["change_pct"] or 0) > 0
            else "下跌" if (stock["change_pct"] or 0) < 0
            else "平盘"
        ...
        lines.append(
            f"{stock['name']}，{direction}百分之{_fmt_num(abs(stock['change_pct'] or 0))}，"
            f"当前判断为{stock['trend']}。"
        )

    lines.extend([
        "第七部分，明日闭环。",
        "明日验证三个问题：第一，今日最强股票是否出现板块扩散；"
        "第二，今日最弱股票是否继续释放风险；"
        "第三，海外映射能否转化为A股实际承接。",
        "明日最需要防止的误判，是把单只股票冲高误认为整个主线转强。",
        "第八部分，原始大盘复盘说明。",
        "完整的大盘指数、涨跌家数和行业板块数据仍保存在原始复盘文件中。"
        "跑步收听版不再整段重复朗读，以减少冗余和逻辑冲突。",
        "以上仅供研究复盘，不构成投资建议。",
    ])
    return to_listening_text("\n".join(lines))
